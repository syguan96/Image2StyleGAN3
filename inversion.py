...
class Inversor():
    ...
    def process_image(self,img, inv=False):
        """
        @inv
            - True: inverse tensor to array, which can be saved directly by PIL
            - False: convert array to tensor
        """
        if not inv:
    ...
    def set_criterions(self,):
        self.MSE_C = nn.MSELoss().cuda()


    def load_G(self, stylegan3_path):
        with open(stylegan3_path, 'rb') as f:
            # import ipdb;ipdb.set_trace()
            self.G = pkl.load(f)['G'].cuda()

    def inverse(self, imgname, total_step, lr, truncation_psi, noise_mode, space='z'):
        gt_image = self.process_image(imgname)
        label = torch.zeros([1, self.G.c_dim]).cuda()
        assert space in ['z', 'w', 'wp']
        code = None
        if space == 'z':
            code = torch.from_numpy(np.random.randn(1, self.G.z_dim)).cuda()
        elif space == 'wp':
            # from avg w
            code = self.G.mapping.w_avg.clone().detach().unsqueeze(0).unsqueeze(0).repeat([1, self.G.num_ws, 1])
            # randomly initialize w
            # code = torch.from_numpy(np.random.randn(1, self.G.w_dim)).cuda()
        ...
        for step_idx in tqdm(range(total_step), total=total_step):
            ...
            # losses
            mseloss = self.MSE_C(inv_image, gt_image)
            optimizer.zero_grad()
            mseloss.backward()
            optimizer.step()

            if step_idx %50 == 0:
                loss_info = 'Step:{} \t MSE:{:0.3f} \n'.format(step_idx, mseloss.item())
        ...
        if space == 'z':
            ...
            inv_image = self.G.synthesis(code, noise_mode=noise_mode)
        inv_image = self.process_image(inv_image, inv=True)
        return inv_image
...
@click.option('--space', type=click.Choice(['z', 'w', 'wp']), default='wp',help='noise mode')
def main(imgname, out_path, stylegan3_path, total_step, lr, truncation_psi, noise_mode, space):
    logpath = osp.join(out_path, 'log.txt')
    inversor = Inversor(stylegan3_path, logpath)
    inv_image = inversor.inverse(imgname, total_step, lr, truncation_psi, noise_mode, space)
